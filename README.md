...
3. 在工具里添加bin目录下的 uart_mqtt_vx.x.x_xxxbps_xMB_XXX.bin 文件(x.x.x为版本号，xxxbps为串口波特率,xMB为要烧写模块flash大小,最后的XXX为要烧写模块FLASH SPI模式)，烧写地址设置为0x0（此bin文件已经将bootloader以及分区表和固件程序打包在一起），选择串口并烧写。详细的添加bin文件以及烧写步骤请参照[如何为 ESP 系列模组烧录固件](http://wiki.ai-thinker.com/esp_download)。关于烧写时参数的配置请注意，需要对应bin文件名称中的模块flash大小以及模块FLASH SPI模式。
...
| ----- | ----- | ----- | ----- | 
| 校验失败 | 操作失败 | "uart_err" | "bad data" |
| 消息类型错误 | 操作失败 | "uart_err" | "type err" |
...
| ----- | ----- | ----- | ----- | 
| 获得IP地址 | 操作成功 | "success" | "wifi suc" |
| 连接丢失 | 操作失败 | "wifi_err" | "wifi err" |
...
| ----- | ----- | ----- | ----- | 
| 连接服务器成功 | 操作成功 | "success" | "serv suc" |
| 客户端初始化失败 | 操作失败 | "mqtt_err" | "init err" |
| 客户端内存分配失败 | 操作失败 | "mqtt_err" | "mem err" |
| 客户端后台进程启动失败 | 操作失败 | "mqtt_err" | "task err" |
| 客户端失去网络连接 | 操作失败 | "mqtt_err" | "serv err" |
...
| ----- | ----- | ----- | ----- | 
| 发布成功 | 操作成功 | "success" | "pub suc" |
| 发布失败 | 操作失败 | "mqtt_err" | "pub err" |
...
| 结果 | 消息类型 | 话题字符串内容 | 消息字符串内容 |
| ----- | ----- | ----- | ----- | 
| 订阅成功 | 操作成功 | "success" | "sub suc" |
| 订阅失败 | 操作失败 | "mqtt_err" | "sub err" |
| 要订阅的话题已经存在 | 操作失败 | "mqtt_err" | "sub exist" |
| 订阅话题数达到最大 | 操作失败 | "mqtt_err" | "sub max" |
...
| 结果 | 消息类型 | 话题字符串内容 | 消息字符串内容 |
| ----- | ----- | ----- | ----- | 
| 取消订阅成功 | 操作成功 | "success" | "unsub suc" |
| 取消订阅失败 | 操作失败 | "mqtt_err" | "unsub err" |
| 要取消订阅的话题不存在 | 操作失败 | "mqtt_err" | "unsub exist" |